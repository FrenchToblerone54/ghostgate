...
def create_sub(comment=None, data_gb=0, days=0, ip_limit=0, sub_id=None):
    sub_id = sub_id or generate(size=20)
    expire_at = (datetime.now(timezone.utc) + timedelta(days=days)).isoformat() if days > 0 else None
    with _conn() as c:
        c.execute(
            "INSERT INTO subscriptions (id, comment, data_gb, days, ip_limit, expire_at) VALUES (?,?,?,?,?,?)",
            (sub_id, comment, data_gb, days, ip_limit, expire_at)
        )
    return sub_id
...
def update_sub(sub_id, **kwargs):
    allowed = {"comment", "data_gb", "days", "ip_limit", "used_bytes", "expire_at"}
    fields = {k: v for k, v in kwargs.items() if k in allowed}
...
def log_access(sub_id):
    with _conn() as c:
        c.execute("INSERT INTO access_logs (sub_id) VALUES (?)", (sub_id,))
...
def get_stats(sub_id):
    with _conn() as c:
        ...
        last = c.execute("SELECT MAX(accessed_at) FROM access_logs WHERE sub_id=?", (sub_id,)).fetchone()[0]
        nodes = c.execute(
            "SELECT n.name FROM subscription_nodes sn JOIN nodes n ON sn.node_id=n.id WHERE sn.sub_id=?", (sub_id,)
        ).fetchall()
        return {**dict(sub), "access_count": count, "first_access": first, "last_access": last, "nodes": [r[0] for r in nodes]}
